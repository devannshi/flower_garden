...
class GreedyGardener(Gardener):
    ...
    def _generate_candidates(self) -> list[Position]:
        ...
        if len(self.garden.plants) == 0:
            ...
            # Get prioritized varieties (already filtered by species)
            prioritized = self._prioritize_varieties()
            
            candidates = []
            
            # Generate candidates for each available species
            for variety in prioritized:
                # Skip same species as first plant
                if variety.species == first_plant.variety.species:
                    continue
    ...
    def _generate_multi_species_candidates(self, variety: PlantVariety) -> list[Position]:
            ...
            for species2 in different_species[i + 1 :]:
                ...
                for p1 in plants1:
                    for p2 in plants2:
                        ...
                        # Use fewer factors to reduce candidates
                        ...
                        # Add tangency candidates around boundary plants only
                        # Sample positions at interaction distance from each plant
                        for angle_idx in range(8):  # Reduced from 12 to 8
                            angle = 2 * 3.14159 * angle_idx / 8
        ...
        if self.config['debug']['verbose'] and candidates:
            print(f'  Generated {len(candidates)} multi-species interaction candidates', end='')
        
        # Filter out interior candidates (keep only boundary candidates)
        boundary_candidates = self._filter_boundary_candidates(candidates, variety)
        
        if self.config['debug']['verbose'] and candidates:
            print(f' → {len(boundary_candidates)} on boundary')
    ...
    def _filter_boundary_candidates(self, candidates: list[Position], variety: PlantVariety) -> list[Position]:
        ...
        Interior candidates (surrounded by plants) are removed.
        
        A candidate is on the boundary if:
        1. It can interact with at least one existing plant
        2. It's not completely surrounded (has exposure to empty space)
        """
        ...
        for pos in candidates:
            # Check if this position has exposure to empty space (not enclosed)
            # We check 8 directions around this position
            # If most directions are blocked by plants, it's interior; otherwise it's boundary
            ...
            for plant in self.garden.plants:
                ...
                # Count nearby plants (within interaction range)
                interaction_range = plant.variety.radius + variety.radius
                if dist < interaction_range:
                    nearby_plants += 1
            
            # Check which directions are blocked by plants
            check_distance = variety.radius * 3  # Look 3 radii away
            
            for dx, dy in directions:
                # Point in this direction
                check_x = pos.x + dx * check_distance
                check_y = pos.y + dy * check_distance
                
                # Check if any plant blocks this direction
                direction_blocked = False
                for plant in self.garden.plants:
                    # Distance from plant to check point
                    dist_to_check = ((check_x - plant.position.x)**2 + (check_y - plant.position.y)**2)**0.5
                    
                    # If plant is close to this check point, direction is blocked
                    if dist_to_check < plant.variety.radius * 2:
            ...
            # A position is on boundary if:
            # 1. Has at least one nearby plant (can interact)
            # 2. Not too many directions are blocked (< 6 out of 8 = has exposure)
            if nearby_plants > 0 and blocked_count < 6:
        ...
        for position_attempt in range(max_position_attempts):
            ...
            if final_size >= 3:
                ...
                if first_plant_position is not None:
                    self.attempted_new_group_positions.append((int(first_plant_position.x), int(first_plant_position.y)))
    ...
    def _find_best_placement_for_new_group(self, candidates: list[Position], new_group_start_idx: int) -> tuple:
        ...
        if new_group_size < 3:
            ...
            available = [v for v in prioritized_varieties if v.species not in existing_species_in_new_group]
